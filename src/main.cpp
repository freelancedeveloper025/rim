...
#if defined(NDEBUG)
#error "Rim cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 168192000 * COIN;
	} else if (nHeight <= 197100 ) {
	        nSubsidy = 80 * COIN;
	} else {
			int tmp = nHeight / 197100 ;
			tmp = (100 - (tmp * 4 ) ) / 100 ;
	        nSubsidy = 80 * tmp * COIN;
	}
...
static CBlockIndex* FindMostWorkChain()
{
    do {
        ...
        while (pindexTest && !chainActive.Contains(pindexTest)) {
            ...
            // for the most work chain if we come rimoss them; we can't switch
